...
#es lo que genera el lenguaje DOT
...
class DotRender(Visitor):
    ...
    def visit(self, node : Block):
        ...
        return name
    # para imprimir el continue en el arbol de expresiones
    def visit(self, node : ContinueStmt):
        name = self.name()
        self.dot.node(name, label='ContinueStmt')
        return name

    def visit(self, node : BreakStmt):
        name = self.name()
        self.dot.node(name, label='BreakStmt')
        return name
