...
class Context:
	...
	def __init__(self):
		self.lexer  = Lexer(self) #analizador lexico
		self.parser = Parser(self) #analizador sintactico
		self.interp = Interpreter(self) #interprete
		self.source = ''
		self.ast    = None
		self.have_errors = False

	def parse(self, source): #hace que funcione el parser
		self.have_errors = False
		self.source = source
		self.ast = self.parser.parse(self.lexer.tokenize(self.source))

	def run(self): #hace que funcione el interprete
		if not self.have_errors:
			return self.interp.interpret(self.ast)

	def find_source(self, node): #busca el nodo en el source
	...
	def error(self, position, message): #imprime el error
